...
export default async function getSlides() {
  if (NODE_ENV === 'production') {
    ...
    await fs.writeFile(path.join(__dirname, '../slides.tsx'), `import { SlideType } from './types';
...
async function getPresentation() {
  ...
  const results = await Promise.all(slides?.map(async (slide: googleapis.slides_v1.Schema$Page): Promise<SlideType | undefined> => {
    if (slide) {
      ...
      const textString = text?.textElements?.map(t => t?.textRun?.content).join('') || '';
      ...
      const buffer = contentUrl && imageData && Buffer.from(await imageData?.arrayBuffer());
